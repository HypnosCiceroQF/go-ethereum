...
echo "==> create node1 account (EMPTY password in keystore)"
PASSFILE="$CLIQUE_DIR/empty.pass"
# 关键：必须是“空密码 + 换行”，不是 0 bytes 文件
printf "\n" > "$PASSFILE"
...
echo "==> write genesis (clique signer = node1 only)"
cat > "$GENESIS" <<JSON
{
  "config": {
    "chainId": $CHAINID,
    ...
    "blobSchedule": {
      ...
      "prague": { "target": 3, "max": 6, "baseFeeUpdateFraction": 3338477 },
      "osaka":  { "target": 3, "max": 6, "baseFeeUpdateFraction": 3338477 }
    }
  },
  "difficulty": "0x1",
  "gasLimit": "0x2fefd8",
